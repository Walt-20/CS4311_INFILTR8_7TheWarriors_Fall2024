		...
		uploadedfiles.forEach(async (filepath) => {
			try {
				const response = await fetch('http://localhost:5001/discard', {
					...
					body: JSON.stringify({ filepath: filepath, userId: userId }),
				});
...
<div class="py-6 text-center">
	<h1 class={`text-2xl font-semibold ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>{greeting}, Analyst!</h1>
</div>

<div class="grid grid-cols-2 gap-6 p-6">
	<div class={`create-project ${$colorBlindMode ? 'color-blind-section' : ''}`}>
		<div class="mb-4 rounded bg-gray-50 p-4 shadow dark:bg-gray-700">
			<h2 class={`flex items-center text-xl font-bold ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>
		...
		<br>
		<div
			class={`file-upload cursor-pointer rounded border-2 border-dashed ${$colorBlindMode ? 'color-blind-border' : 'border-gray-300'} p-6 text-center hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-500 dark:hover:bg-gray-600`}
		>
			<div class="flex flex-col items-center justify-center space-y-4">
				<h2 class={`text-m flex items-center ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>Drag and Drop a File Here Or</h2>
	...
	<div class={`projects ${$colorBlindMode ? 'color-blind-section' : ''}`}>
		<div class="mb-4 rounded bg-gray-50 p-4 shadow dark:bg-gray-700">
			<h2 class={`flex items-center text-xl font-bold ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>
		...
		<div class={`rounded p-4 shadow ${$colorBlindMode ? 'color-blind-section' : 'bg-white dark:bg-gray-700'}`}>
			{#if projects.length === 0}
				<p>No Projects Available</p>
			{:else}
				{#each projects as project}
				<h2 class={`cursor-pointer mb-2 flex items-center text-md font-bold transition transform hover:scale-105 hover:shadow-xl ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>
	...
	<div class="upload-files col-span-2 mt-6">
		...
		{#if files.length > 0}
			<h2 class={`mt-6 text-xl font-bold ${$colorBlindMode ? 'color-blind-text' : 'dark:text-gray-200'}`}>Uploading Files</h2>
